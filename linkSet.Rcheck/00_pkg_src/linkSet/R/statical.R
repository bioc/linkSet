...
#'
#' @description
#'	Split data frame into roughly equal sized bins based on a column.
#'	Avoids splitting up rows with identical values in the column.
#'
#' @param dat Data table to split
#' @param bins Number of bins to split into
#' @rdname chicane
#' @return List of data.tables, where each element corresponds to a bin
#'	of the data  
#'
smartSplit <- function(dat, bins) {
	...
	if( !is.data.table(dat) ) {
		stop('dat must be a data.table object');
	}
	...
	split.col <- 'distance';
	if( !split.col %in% names(dat) ) {
		stop('dat must contain a column named distance');
	}

	# get unique values of the column to split on
	split.values <- sort(unique(dat[[split.col]]));
	n.unique <- length(split.values);

	# if there are fewer unique values than bins, just split by unique values
	if( n.unique <= bins ) {
		split.data <- split(dat, dat[[split.col]]);
		return(split.data);
	}

	# otherwise, split into roughly equal sized bins
	bin.size <- ceiling(n.unique / bins);
	bin.indices <- as.integer(gl(ceiling(n.unique/bin.size), bin.size, n.unique));
	
	# create a mapping from split value to bin
	split.map <- data.table(
		split.value = split.values,
		bin = bin.indices
		);

	# add bin column to data
	dat <- merge(dat, split.map, by.x = split.col, by.y = 'split.value');
	
	# split by bin
	split.data <- split(dat, dat$bin);
	
	# remove bin column from each element
	split.data <- lapply(split.data, function(x) { x$bin <- NULL; return(x); });
...
#'
#' @description
#' 	Check if chicane model can be fit on a list of datasets.
#'
#' @param split.data List of data tables of interaction data on which model is to be fit
#' @rdname chicane
#' @return boolean indicating if model can be fit on all datasets
#' 
checkSplitDataNumericalFit <- function(split.data) {
	...
	if( !is.list(split.data) ) {
		stop('split.data must be a list of data.table objects');
	}

	if( length(split.data) == 0 ) {
		stop('split.data must be a non-empty list');
	}

	if( !all(sapply(split.data, is.data.table)) ) {
		stop('split.data must be a list of data.table objects');
	}

	### MAIN ##################################################################

	# check if model can be fit on each dataset
	fit.check <- sapply(split.data, checkModelNumericalFit);
	
	return(all(fit.check));
}

#' verify.interaction.data
#'
#' @description
#'	Check that interaction data is a data.table with the required columns.
#'
#' @param interaction.data Data table of interaction calls
#' @rdname chicane
#' @return None
#' 
verifyInteractionData <- function(interaction.data) {
	...
	required.columns <- c('bait.id', 'target.id');
	if( !all(required.columns %in% names(interaction.data)) ) {
		stop('interaction.data must contain columns bait.id and target.id');
	}
}
...
modelRowsSanityCheck <- function(model.data, model) {
	...
	if( nrow(model.data) != length(model$fit) ) {
		...
		error.message <- paste(
			...
			'Model fit contains', length(model$fit), 'rows\n',
			'This is probably due to missing values in the data, but should have been caught earlier'
			);
		stop(error.message);
	}
}

#' isGlmNbMaxiterWarning
#'
#' @description
