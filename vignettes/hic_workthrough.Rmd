...
```{r}
ls <- filterLinks(ls,filter_intra = TRUE, filter_unannotate = TRUE, distance = 500000)
```
...
orderLinks(ls,by = "crossFreq",decreasing = T)
```

```{r}
plot_genomic_ranges(ls,showBait = "Ncoa2",minimal_width = 0.005)
```
